...
class MigrationNotice
{
    public function __construct()
    {
        ...
        add_action('admin_enqueue_scripts', [$this, 'enqueue_migration_script']);
    ...
    private function should_show_notice()
    {
        ...
        return isset($options['tailwind_version']) && $options['tailwind_version'] === 'v3';
    }

    /**
     * Enqueue migration notice script
     * WordPress.org requirement: Use wp_add_inline_script instead of echo
     */
    public function enqueue_migration_script()
    {
        ...
        $nonce = wp_create_nonce('winden_dismiss_v4');

        $inline_script = sprintf(
            '(function() {
                var ajaxUrl = %s;
                var nonce = %s;
                var notice = document.getElementById("winden-v4-notice");
                if (notice) {
                    ...
                    // Handle copy button
                    var copyButton = document.getElementById("copy-v4-code");
                    var codeSnippet = document.getElementById("v4-code-snippet");

                    if (copyButton && codeSnippet) {
                        copyButton.addEventListener("click", function() {
                            var textToCopy = codeSnippet.textContent;
        ...
        wp_add_inline_script('winden-migration-notice', $inline_script);
    }

    /**
     * Show v4 migration notice
     * Note: Inline script is handled by enqueue_migration_script() method
     */
