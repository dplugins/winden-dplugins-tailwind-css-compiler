...
class ProvidersHelpers
{
     ...
     * Note: This returns a static hardcoded function definition (no user input)
     * Safe for inline script output as content is fully controlled by plugin
     *
     * @return string JavaScript function definition
     */
    public static function get_autocomplete_js()
    {
        // phpcs:disable WordPress.Security.EscapeOutput.OutputNotEscaped -- Static hardcoded JS function, no user input
        return "
    ...
    public static function load_compiled_css($reorder = true)
    {
        ...
        if (file_exists($css_file_path)) {
            if ($inline_css) {
                ...
                // phpcs:ignore WordPress.WP.AlternativeFunctions.file_get_contents_file_get_contents -- Reading local file from uploads directory
                $css_content = file_get_contents($css_file_path);
                if ($css_content !== false) {
                    // Sanitize CSS: strip any HTML tags that could break out of <style> context
                    // The CSS is generated by Tailwind compiler from admin-controlled input
